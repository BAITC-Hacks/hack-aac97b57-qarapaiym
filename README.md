...
**Движок считает, AI объясняет.** Данные синтетические; это не официальная статистика Астаны и не прогноз реальных последствий. AI-текст может содержать ошибки — источник численных результатов находится в таблицах расчёта.
...
| Поддержка решений | Бюджет, показатели до/после по направлениям и районам, компоненты формулы, вклад каждой меры, сравнение A/B |
...
flowchart LR
    ...
    AI --> CHECK["Проверка JSON и evidence"]
    CHECK --> TEXT["AI-объяснение в UI"]
    CHECK -->|Ошибка| ERROR
```
...
Сервер проверяет отдельный блок `evidence`, но **это не доказывает истинность каждого предложения**. Неверное число или неподтверждённая причинная связь всё ещё могут появиться в свободном тексте. Сверяйте объяснение с таблицами; не используйте предложенный AI вариант без повторного расчёта движком.
...
│   ├── components/            # Вспомогательный модуль initiative-copy
...
|---|---|
| `npm test` | 27 тестов текущей версии: модель, цели/правила, синергии, ошибки, диагностика/альтернативы, серверная проверка и ответы AI с подставным провайдером |
...
AI-тесты не отправляют платные запросы. Успешные тесты не гарантируют фактическую точность свободного AI-текста. Свежие условия и результаты проверки README: [docs/README_VERIFICATION.md](docs/README_VERIFICATION.md).
...
3. Раскройте «По районам: до → после» и «Вклад мероприятий и синергии». Проверьте S2 Нуры **43,75** и бонус M10+M12.
4. Нажмите «Сохранить для сравнения». В плане поменяйте район **M7 на Есиль**. Старое объяснение и результат должны исчезнуть. Пересчитайте: **55,30**, расход по-прежнему **95**; появится сравнение A/B.
...
→ проверка JSON/evidence → summary, strengths, risks, tradeoffs, recommendations
```

Успех: HTTP 200, пять полей анализа. `recommendations` содержит `title`, `rationale`, `category`. Внутренний provider-evidence удаляется перед ответом UI. Ключ в ответ не включается.
...
- AI проверяется по схеме и отдельному evidence; фактическая точность всей прозы **не гарантирована**.
...
- Диагностика расходов и поиск альтернатив реализованы библиотечными модулями и тестами, **но не подключены к UI**. Стейкхолдер-анализ и расширенная objective-review панель в текущем main отсутствуют.
...
1. Фактически проверяемые AI-утверждения со ссылками на канонические факты.
2. Подключение существующих диагностики и поиска альтернатив к UI.
...
| Число в AI отличается от таблицы | Известное ограничение свободной прозы | Используйте детерминированную таблицу; повторный AI-запрос не гарантирует исправление |

## FAQ

**AI рассчитывает Score?** Нет. Общий движок TypeScript рассчитывает Score; AI получает готовый результат и может ошибиться только в своём объяснении.
