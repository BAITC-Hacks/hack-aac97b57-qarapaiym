...
[Быстрый старт](#быстрый-старт) · [Установка](#установка-из-чистого-клона) · [Архитектура](#архитектура) · [Проверенные результаты](FINAL_SUBMISSION_VERIFICATION.md) · [Пример за 95 единиц](#эталонный-сценарий) · [Формула](#как-считается-score) · [Ручная проверка](#как-проверить-приложение-вручную) · [Устранение проблем](#устранение-проблем)
...
Зафиксируйте версию командой `git rev-parse HEAD` перед демонстрацией. Старые записи в [docs/VERIFICATION.md](docs/VERIFICATION.md) — исторические проверки, а не автоматическая гарантия текущего состояния. Свежая проверка этой инструкции: [FINAL_SUBMISSION_VERIFICATION.md](FINAL_SUBMISSION_VERIFICATION.md).
