  ...
  return <div className="simulator" lang="ru">
    ...
    <main>
      ...
      {result&&projected&&<section className="results-section" ref={resultsRef} tabIndex={-1} aria-labelledby="results-heading">
        <div className="section-heading"><div><p className="eyebrow">03 / ПОСЛЕДСТВИЯ</p><h2 id="results-heading">Результат сценария</h2></div><div className="result-actions"><button className="secondary-button" onClick={editPlan}>Изменить решения</button><button className="text-button" onClick={()=>setSaved(result)}>Сохранить для сравнения</button></div></div>
        <div className="score-result"><div><span>До решений</span><strong>{score(result.baseline.overall)}</strong></div><ArrowRight/><div className="projected-score"><span>После решений</span><strong>{score(projected.overall)}</strong></div><div className="score-change"><span>Изменение AQoL</span><strong>{signed(result.delta!)}<small> п.</small></strong></div></div>
        <p className="result-caption">Выделено {money(result.budget.spent)}, осталось {money(result.budget.remaining)}. Горизонт: 8 кварталов. Score и прирост округлены независимо, поэтому разность показанных баллов может отличаться на 0,01.</p>
        <div className="table-scroll"><table><caption>Почему изменился рейтинг</caption><thead><tr><th>Компонент формулы</th><th>До</th><th>После</th></tr></thead><tbody>{[{label:"Средневзвешенный районный балл (70%)",a:baseline.weightedAverage,b:projected.weightedAverage},{label:"Худший районный балл (30%)",a:baseline.worstDistrictScore,b:projected.worstDistrictScore},{label:"Штраф: показатели строго ниже 40",a:baseline.criticalCount,b:projected.criticalCount}].map(r=><tr key={r.label}><th>{r.label}</th><td>{number(r.a)}</td><td>{number(r.b)}</td></tr>)}</tbody></table></div>
        <div className="metrics-results"><h3>Что изменилось по направлениям</h3>{CATEGORIES.map(c=><div className="metric-row" key={c}><span>{labels[c]}</span><div className="comparison-bars"><i style={{width:`${baseline.byCategory[c]}%`}}/><i style={{width:`${projected.byCategory[c]}%`}}/></div><span className="metric-values">{score(baseline.byCategory[c])} → <strong>{score(projected.byCategory[c])}</strong></span><strong>{signed(projected.byCategory[c]-baseline.byCategory[c])}</strong></div>)}</div>
        <details className="district-details"><summary>По районам: до → после <ChevronDown size={15}/></summary><DistrictTable snapshot={projected} before={baseline}/></details>
        <details className="consequences"><summary>Вклад мероприятий и синергии <ChevronDown size={15}/></summary><p>Изменения показателей до ограничения 0–100. Эффекты мер суммируются, синергии добавляются отдельно.</p>{result.contributions.map(c=><div className="consequence-row" key={c.initiativeId}><div><h4>{c.initiativeId} · {result.selectedInitiatives.find(i=>i.id===c.initiativeId)?.name}</h4><p>{districtName(c.districtId)} · реализовано {number(c.fraction*100)}%</p></div><ul className="effects">{c.impacts.map((i,k)=><li key={k}><span>{districtName(i.districtId)} · {i.metric}</span><strong>{signed(i.delta)}</strong></li>)}</ul></div>)}<p>{result.synergies.length?"Сработавшие синергии:":"Синергии в этом плане не сработали."}</p>{result.synergies.map((s,k)=><p key={k}>{s.initiativeIds.join(" + ")}: {s.impacts.map(i=>`${districtName(i.districtId)} ${i.metric} ${signed(i.delta)}`).join(", ")}</p>)}</details>
        {saved?.projected&&<section className="comparison"><div className="section-heading"><h3>Сравнение сценариев</h3><button className="text-button" onClick={()=>setSaved(null)}>Убрать сохранённый A</button></div><div className="table-scroll"><table><thead><tr><th>Показатель</th><th>Сохранённый A</th><th>Текущий B</th><th>B − A</th></tr></thead><tbody>{[{label:"AQoL",a:saved.projected.overall,b:projected.overall},{label:"Расход, единицы бюджета",a:saved.budget.spent,b:result.budget.spent},...CATEGORIES.map(c=>({label:labels[c],a:saved.projected!.byCategory[c],b:projected.byCategory[c]}))].map(r=><tr key={r.label}><th>{r.label}</th><td>{number(r.a)}</td><td>{number(r.b)}</td><td>{signed(r.b-r.a)}</td></tr>)}</tbody></table></div><p className="muted">A хранится до перезагрузки страницы. Измените решения и рассчитайте B.</p></section>}
        <section className="analysis-section"><h3>Объяснение и рекомендации</h3><p className="muted">AI объясняет рассчитанные факты. Числа в таблицах — основной результат.</p><div aria-live="polite" aria-busy={loading}>
          {loading&&<div className="loading-state"><LoaderCircle className="spin"/><p>Рейтинг рассчитан. Ожидаем сильные стороны, риски и компромиссы.</p></div>}
          {error&&<div className="error-state" role="alert"><strong>Объяснение пока недоступно. Расчёт сохранён.</strong><p>{error.includes("OPENAI_API_KEY")?"Для команды: настройте ключ на сервере. Результаты симуляции доступны без ключа.":"Не удалось получить ответ. Повторите запрос."}</p><button className="secondary-button" onClick={()=>void explain(result)}>Повторить объяснение</button></div>}
          {analysis&&<><p className="analysis-summary">{analysis.summary}</p><div className="analysis-columns">{[{title:"Сильные стороны",items:analysis.strengths},{title:"Риски",items:analysis.risks},{title:"Компромиссы",items:analysis.tradeoffs}].map(g=><div key={g.title}><h4>{g.title}</h4><ul>{g.items.map((s,i)=><li key={i}>{s}</li>)}</ul></div>)}</div><div className="recommendations"><h4>Что пересмотреть</h4>{analysis.recommendations.map((r,i)=><div key={i}><strong>{r.title}</strong><p>{r.rationale}</p></div>)}</div></>}
        </div></section>
      </section>}
