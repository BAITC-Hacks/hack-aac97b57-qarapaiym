...
import type { AIAnalysis, District, Contribution, Initiative, SimulationResult } from '@/types/city';
import { CATEGORIES, METRICS, METRIC_LABELS } from '@/types/city';
...
function InitiativeRow({ initiative, contribution, districts }: { initiative: Initiative; contribution: Contribution; districts: District[] }) {
  const affected = [...new Set(contribution.impacts.map(impact => impact.districtId))];
  const effects = METRICS.flatMap(category => {
    const values = contribution.impacts.filter(impact => impact.metric === category).map(impact => impact.delta);
  ...
  return <article className="review-initiative">
    ...
    <div className="review-effect-chips">{effects.map(effect => <span key={effect.category}>{METRIC_LABELS[effect.category]} <b className={effect.low < 0 ? 'negative' : 'positive'}>{effect.low === effect.high ? delta(effect.low) : `${delta(effect.low)}…${delta(effect.high)}`} п.</b></span>)}</div>
    <details className="review-detail"><summary>Эффекты по районам <ChevronDown size={14} /></summary><ul className="review-effect-list">{contribution.impacts.map((impact, index) => <li key={index}><span>{districts.find(district => district.id === impact.districtId)?.name ?? impact.districtId} · {METRIC_LABELS[impact.metric]}</span><Change value={impact.delta} /></li>)}</ul></details>
  </article>;
}
...
export function ScenarioReview({ result, districts, saved, analysis, loading, aiError, onSave, onClearSaved, onRetry }: Props) {
  if (!result.valid || !result.projected || result.delta === null) return <p className="error-message" role="alert">Сценарий недействителен. Исправьте решения перед просмотром результата.</p>;
  const projected = result.projected;
  const savedProjected = saved?.projected;
  ...
  return <div className="review-dashboard">
    ...
    <p className="review-footnote">Score и прирост округлены независимо: разность показанных баллов может отличаться на 0,01.</p>
    <dl className="review-kpis">
      <div className="review-kpi review-kpi-main"><dt>Итоговый AQoL</dt><dd>{number(projected.overall)}<small> / 100</small></dd><p>До решений: {number(result.baseline.overall)}</p></div>
      ...
      <div className="review-kpi"><dt>Бюджет, ед. бюджета</dt><dd>{number(result.budget.spent)}<small> / {number(result.budget.total)}</small></dd><p>Остаток: {money(result.budget.remaining)}</p></div>
      <div className="review-kpi"><dt>Критические показатели</dt><dd>{projected.criticalCount}</dd><p>До: {result.baseline.criticalCount} · строго ниже 40</p></div>
    ...
    <div className="review-visual-grid">
      <section className="review-panel" aria-labelledby="review-score-heading"><div className="review-panel-heading"><h3 id="review-score-heading">Как изменился балл</h3><span className="review-tag">AQoL</span></div><Legend /><ComparisonBars rows={[{ id: 'overall', name: 'Качество жизни', before: result.baseline.overall, after: projected.overall, delta: result.delta }]} /></section>
      <section className="review-panel" aria-labelledby="review-budget-heading"><div className="review-panel-heading"><h3 id="review-budget-heading">Распределение бюджета</h3><span className="review-tag">{number(result.budget.total)} ед. бюджета</span></div>
        <div className="review-budget-headline"><strong>{number(result.budget.spent)}<small> ед. бюджета выделено</small></strong><span>Резерв {money(result.budget.remaining)}</span></div>
      ...
      <section className="review-panel" aria-labelledby="review-districts-heading"><div className="review-panel-heading"><h3 id="review-districts-heading">Изменения по районам</h3><span className="review-tag">По приросту</span></div><Legend /><ComparisonBars rows={model.districts} />
        ...
        <p className="review-footnote">Взвешенный балл десяти показателей района из движка. Score учитывает население, худший район и критические значения.</p>
        <details className="review-detail"><summary>Все показатели районов <ChevronDown size={14} /></summary><div className="table-scroll" role="region" aria-label="Показатели районов до и после решений" tabIndex={0}><table><thead><tr><th scope="col">Район</th>{METRICS.map(id => <th scope="col" key={id}>{id} · {METRIC_LABELS[id]}</th>)}</tr></thead><tbody>{projected.districts.map(district => <tr key={district.districtId}><th scope="row">{districts.find(item => item.id === district.districtId)?.name ?? district.districtId}</th>{METRICS.map(id => <td key={id}>{number(result.baseline.districts.find(item => item.districtId === district.districtId)!.metrics[id])} → {number(district.metrics[id])}</td>)}</tr>)}</tbody></table></div></details>
      </section>
    </div>

    <details className="review-detail"><summary>Компоненты формулы Score <ChevronDown size={14}/></summary><div className="table-scroll"><table><thead><tr><th>Компонент</th><th>До</th><th>После</th></tr></thead><tbody>{[{name:'Средневзвешенный балл (70%)',before:result.baseline.weightedAverage,after:projected.weightedAverage},{name:'Худший район (30%)',before:result.baseline.worstDistrictScore,after:projected.worstDistrictScore},{name:'Критические значения (штраф)',before:result.baseline.criticalCount,after:projected.criticalCount}].map(r=><tr key={r.name}><th>{r.name}</th><td>{number(r.before)}</td><td>{number(r.after)}</td></tr>)}</tbody></table></div></details>
    <section className="review-evidence" aria-labelledby="review-initiatives-heading"><div className="review-panel-heading"><div><p className="eyebrow">ОСНОВАНИЕ РАСЧЁТА</p><h3 id="review-initiatives-heading">За счёт чего меняется результат</h3></div><span className="review-tag">{result.selectedInitiatives.length} инициатив</span></div>
      <p className="review-footnote">Заданные эффекты по районам до ограничения 0–100. Диапазоны не являются вкладом в общий AQoL. Лаги уже учтены; синергии показаны отдельно.</p>
      <div className="review-initiatives">{result.contributions.map(contribution => { const item = result.selectedInitiatives.find(i => i.id === contribution.initiativeId); return item ? <InitiativeRow key={item.id} initiative={item} contribution={contribution} districts={districts} /> : null; })}</div>
      <p>Синергии: {result.synergies.length ? result.synergies.map(s => `${s.initiativeIds.join(" + ")}: ${s.impacts.map(i => `${districts.find(d => d.id === i.districtId)?.name} ${i.metric} ${delta(i.delta)}`).join(", ")}`).join("; ") : "не сработали"}</p>
    </section>

    {saved?.valid && savedProjected && <section className="review-panel review-saved" aria-labelledby="review-saved-heading"><div className="review-panel-heading"><h3 id="review-saved-heading">Сравнение с сохранённым A</h3><button className="text-button" onClick={onClearSaved}>Убрать A</button></div><div className="table-scroll" role="region" aria-label="Сравнение сценариев A и B" tabIndex={0}><table><thead><tr><th scope="col">Показатель</th><th scope="col">A</th><th scope="col">Текущий B</th><th scope="col">B − A</th></tr></thead><tbody>{[{ id: 'aqol', name: 'AQoL', a: savedProjected.overall, b: projected.overall }, ...CATEGORIES.map(id => ({ id, name: directionLabels[id], a: savedProjected.byCategory[id], b: projected.byCategory[id] })), { id: 'budget', name: 'Выделено, ед. бюджета', a: saved.budget.spent, b: result.budget.spent }].map(row => <tr key={row.id}><th scope="row">{row.name}</th><td>{number(row.a)}</td><td>{number(row.b)}</td><td>{delta(difference(row.b, row.a))}</td></tr>)}</tbody></table></div><p className="review-footnote">Сохранённый A доступен до перезагрузки страницы.</p></section>}
    ...
    <section className="review-ai" aria-labelledby="analysis-heading"><div className="review-panel-heading"><div><p className="eyebrow">ИНТЕРПРЕТАЦИЯ</p><h3 id="analysis-heading">Что учесть при решении</h3></div><span className="review-tag">AI-разбор</span></div>
      <p className="review-footnote">AI выбирает акценты из проверенных выводов. Формулировки и рекомендации связаны с расчётом; новые факты AI не добавляет.</p>
