...
export function calculateScenarioDiagnostics(simulation: SimulationResult): ScenarioDiagnostics {
  if (!simulation.valid) throw new Error("Scenario diagnostics require a valid simulation");
  ...
  const categoryDeltas = Object.fromEntries(CATEGORIES.map(category => [category, round(simulation.projected.byCategory[category] - simulation.baseline.byCategory[category], 1)])) as Record<Category, number>;
  const strongest = CATEGORIES.reduce((best, category) => categoryDeltas[category] > categoryDeltas[best] ? category : best, CATEGORIES[0]);
  const weakest = CATEGORIES.reduce((worst, category) => simulation.projected.byCategory[category] < simulation.projected.byCategory[worst] ? category : worst, CATEGORIES[0]);
  const largest = CATEGORIES.reduce((best, category) => spending[category] > spending[best] ? category : best, CATEGORIES[0]);
  const baselineDistricts = new Map(simulation.baseline.districts.map(district => [district.districtId, district.metrics]));
  const districtImprovements = simulation.projected.districts.map(district => {
    const before = baselineDistricts.get(district.districtId);
    if (!before) throw new Error("Projected district is absent from baseline");
    const delta = CATEGORIES.reduce((sum, category) => sum + district.metrics[category] - before[category], 0) / CATEGORIES.length;
  ...
  return {
    ...
    aqolGainPer100M: spent ? round(simulation.delta / spent * 100, 2) : null,
    strongestImprovement: { category: strongest, delta: categoryDeltas[strongest] },
    weakestFinalCategory: { category: weakest, score: simulation.projected.byCategory[weakest] },
